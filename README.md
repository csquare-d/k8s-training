...
Make sure you've started a new terminal or sourced your shell config:
...
All pods should be in `Running` state. If not, check the logs:
...
This usually means the CNI isn't fully installed yet. Wait a minute and check again. If it persists, check Calico pod logs.

### Permission denied errors

The setup script should be run as a regular user with sudo access, not as root directly:

```bash
# Wrong
sudo ./setup-k3s-calico.sh
...
k3s doesn't include metrics-server by default. If you want `kubectl top` to work:
